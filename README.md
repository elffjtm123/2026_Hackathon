...
- 개발용 스타일 미리보기: `POST /api/v1/scripts/style-transfer/demo`
...
`/ws/practice-demo`와 `/scripts/style-transfer/demo`는 개발·테스트 전용이며 운영
환경에서는 비활성화됩니다. 데모 WebSocket도 `session.start`를 받은 뒤 대본과
제한시간으로 파이프라인을 만들며, 이후 미디어 timestamp는 세션 시작 후 경과
밀리초를 사용합니다. 프론트는 실제 STT가 연결되기 전까지 전체 대본을 발화로
자동 전송하지 않습니다.
